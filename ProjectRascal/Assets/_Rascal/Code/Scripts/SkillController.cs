...
public class SkillController : MonoBehaviour
{
    ...
    public IEnumerator WaitForFireball(float delay, Vector3 mouseGroundPosition)
    {
        yield return new WaitForSeconds(delay);
        SpawnFireball(mouseGroundPosition);
    }

    public IEnumerator WaitForAbyssProjectile(float delay, Vector3 mouseGroundPosition)
    {
        yield return new WaitForSeconds(delay);
        SpawnAbyssProjectile(mouseGroundPosition);
    }

    public void SpawnFireball(Vector3 mouseGroundPosition) {
        var spawnTransform = bulletSpawnPoint.transform;
        var bullet = DamageDealerWizard.instance.SummonFireball(spawnTransform.position, spawnTransform.rotation);
        if(bullet.TryGetComponent(out FireballDD damageDealer)) {
            damageDealer.FeedAndDealDamage(ownerCharacter: gameCharacter, endPoint: mouseGroundPosition, damageStartTime: 0f, damageDuration: 5f);
            damageDealer.SetLifetime();
        }
    }

    public void SpawnAbyssProjectile(Vector3 mouseGroundPosition) {
        var spawnTransform = bulletSpawnPoint.transform;
        var bullet = DamageDealerWizard.instance.SummonAbyssProjectile(spawnTransform.position, spawnTransform.rotation);
        if(bullet.TryGetComponent(out FireballDD damageDealer)) {
            damageDealer.FeedAndDealDamage(ownerCharacter: gameCharacter, endPoint: mouseGroundPosition, damageStartTime: 0f, damageDuration: 5f);
            damageDealer.SetLifetime();
        }
